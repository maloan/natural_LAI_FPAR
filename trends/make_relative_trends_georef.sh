...
#   r(i) [yr^-1] = b(i) / X̄(i),
#   where b(i) is the OLS slope (per year) and X̄(i) is the temporal mean level
#   of the corresponding metric time series. r(i) is only defined where
#   X̄(i) >= EPS (variable-specific).
#
# Usage
