...
get_fractional_used_land <- function(cci_stack, years, tmpl, band) {
  # Mean fractional used-land across all selected years
  frac_mean <- app(cci_stack[[band]], fun = mean, na.rm = TRUE)

  # Ensure geometry
  if (!compareGeom(frac_mean, tmpl, stopOnError = FALSE)) {
    frac_mean <- resample(frac_mean, tmpl, method = "bilinear")
  }

  # Natural fraction: 1 - used_land_fraction
  w_nat <- 1 - frac_mean

  # Bound to [0,1]
  w_nat <- clamp(w_nat, 0, 1)
