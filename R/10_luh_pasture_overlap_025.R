...
grass_005 <- switch(GRASS_SOURCE, "CCI" = {
  ...
  grass_vals <- unlist(cfg$glc$classes$grassland, use.names = FALSE)
...
if (all(is.na(ty))) {
  stop("LUH time axis missing/NA for ", v_pas)
}
...
out005 <- file.path(out_dir, sub("_0p25\\.tif$", "_0p05_rep.tif", basename(out025)))
...
for (nm in names(aois)) {
  ...
  ql_aoi <- file.path(aoi_dir,
                      sprintf(
                        "luh_overlap_%s_%s_%s_%s.png",
                        GRASS_SOURCE,
                        tok(G_MIN),
                        tok(P_MIN),
                        nm
  ...
  if (REMAKE_QL || !file.exists(ql_aoi)) {
    ...
    mtext(
      sprintf("LUH Pasture-Overlap Mask — AOI: %s — %d–%d", nm, Y0, Y1),
      outer = TRUE,
      line = 0,
      cex = 1.3
    )
