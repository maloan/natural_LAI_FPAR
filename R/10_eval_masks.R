## =============================================================================
# 10_eval_masks.R — Evaluate and cross-compare masking performance (QA & stats)
#
# Purpose
#   Perform global and AOI-level quality assessment, compute area-weighted
#   masked fractions, cross-compare different mask families (CCI, GLC, LUH),
#   and quantify sensitivity to threshold parameters (τ, k, N, α, Gmin, Pmin).
#
# Inputs
#   - Mask rasters (0.05°):
#       * CCI → mask_used_*_tau*_k*_*_0p05.tif
#       * GLC → mask_used_ge*_0p05.tif
#       * LUH → m_LUH_pasture_*_0p05_rep.tif
#   - Cell area raster: cfg$grids$grid_005$area_raster
#   - AOI polygons (optional): cfg$aois or cfg$project$aois
#
# Outputs
#   - Quicklooks (global + AOI) for each mask
#   - global_masked_fraction.csv — area-weighted fraction masked (per mask)
#   - pairwise_comparison_area_weighted.csv — Jaccard and contingency stats
#   - sensitivity_cci_tau_k.csv — τ,k sweep results for CCI masks
#   - sensitivity_glc_N.csv — N sweep results for GLC masks
#   - sensitivity_luh_alpha_gmin_pmin.csv — α,Gmin,Pmin sensitivity table
#   - mean_{lai|fpar}_{mask}_masked_vs_unmasked.csv — mean values split by mask
#
# Dependencies
#   Packages: terra, yaml, stringr, glue
#   Sourced helpers: utils.R, io.R, geom.R, viz.R, options.R
#
# Processing overview
#   1) Load CCI, GLC, LUH masks; align to 0.05° grid.
#   2) Generate global and AOI quicklooks (1=drop, 0=keep).
#   3) Compute global area-weighted masked fractions.
#   4) Pairwise compare masks (CCI–GLC–LUH) via Jaccard index.
#   5) Sensitivity analyses for τ,k (CCI), N (GLC), α,Gmin,Pmin (LUH).
#   6) Evaluate mean LAI/FPAR in masked vs unmasked regions.
## =============================================================================
...
QLDIR  <- file.path(OUTDIR, "quicklooks")
dir.create(QLDIR, TRUE, showWarnings = FALSE)
...
cci_files <- list.files(cci_dir,
                        "mask_used_frac_fused_tau0p10_k3_1992-2020_0p05.tif",
                        full.names = TRUE)
...
luh_files <- list.files(
  luh_dir,
  "m_LUH_pasture_1992-2020_0p05_rep.tif",
  full.names = TRUE,
  ignore.case = TRUE
)
...
# Global quicklooks
plot_mask(m_cci,
          "CCI used-mask (1=drop)",
          file.path(QLDIR, "mask_cci_global.png"))
plot_mask(m_glc,
          "GLC used≥N mask (1=drop)",
          file.path(QLDIR, "mask_glc_global.png"))
plot_mask(m_luh,
          "LUH overlap mask (1=drop)",
          file.path(QLDIR, "mask_luh_global.png"))
...
write.csv(cmp,
          file.path(OUTDIR, "pairwise_comparison_area_weighted.csv"),
          row.names = FALSE)

message(
  "Jaccard (area-weighted, on drop=1): ",
  glue(
    "CCI~GLC={sprintf('%.3f', cmp$jacc[cmp$pair=='CCI_vs_GLC'])}, "
  ),
  glue(
    "CCI~LUH={sprintf('%.3f', cmp$jacc[cmp$pair=='CCI_vs_LUH'])}, "
  ),
  glue(
    "GLC~LUH={sprintf('%.3f', cmp$jacc[cmp$pair=='GLC_vs_LUH'])}"
  )
)

# ==== 4) Sensitivity analyses (read masks already produced) ====================
# 4a) CCI: vary τ and k (scan files present)
cci_all <- list.files(cci_dir,
                      "^mask_used_.*_tau.*_k\\d+_\\d{4}-\\d{4}_0p05\\.tif$",
                      full.names = TRUE)
if (length(cci_all)) {
  parse_tau  <- function(s)
    as.numeric(sub("p", ".", str_match(s, "_tau([0-9p]+)_")[, 2]))
  parse_k    <- function(s)
    as.integer(str_match(s, "_k(\\d+)_")[, 2])
  df <- data.frame(
    path = cci_all,
  ...
  for (i in seq_len(nrow(df))) {
    r <- read_mask_safe(df$path[i])
    df$p_drop[i] <- p_drop(r)
  }
  write.csv(df,
            file.path(OUTDIR, "sensitivity_cci_tau_k.csv"),
            row.names = FALSE)
}

# 4b) GLC: vary N (scan files present)
glc_all <- list.files(glc_dir,
                      "^mask_used_ge\\d+_\\d{4}-\\d{4}_0p05\\.tif$",
                      full.names = TRUE)
if (length(glc_all)) {
  parse_N <- function(s)
    as.integer(str_match(s, "mask_used_ge(\\d+)_")[, 2])
  dg <- data.frame(path = glc_all,
                   N = parse_N(glc_all),
                   stringsAsFactors = FALSE)
  dg <- dg[order(dg$N), ]
  dg$p_drop <- NA_real_
  for (i in seq_len(nrow(dg))) {
    r <- read_mask_safe(dg$path[i])
    dg$p_drop[i] <- p_drop(r)
  }
...
luh_all <- unique(c(
  ...
  list.files(
    file.path(cfg$paths$masks_root_dir),
    ".*0p05.*\\.tif$",
    full.names = TRUE,
    recursive = TRUE
  )
...
if (length(luh_all)) {
  ex <- function(re, s) {
    m <- str_match(s, re)[, 2]
    suppressWarnings(as.numeric(sub("p", ".", m)))
  }
  dl <- data.frame(
    path = luh_all,
  ...
  if (nrow(dl)) {
    ...
    for (i in seq_len(nrow(dl))) {
      r <- read_mask_safe(dl$path[i])
      dl$p_drop[i] <- p_drop(r)
    }
    dl <- dl[order(dl$alpha, dl$gmin, dl$pmin, na.last = TRUE), ]
    write.csv(dl,
              file.path(OUTDIR, "sensitivity_luh_alpha_gmin_pmin.csv"),
              row.names = FALSE)
  }
}
...
for (VAR in c("FPAR", "LAI")) {
  for (MASK_NAME in c("CCI", "GLC")) {
    masked_dir <- cfg$paths[[paste0("masked_",
                                    tolower(VAR),
                                    "_",
                                    tolower(MASK_NAME),
                                    "_005_dir")]]
    masked_files <- list.files(
      masked_dir,
      pattern = paste0("^", VAR, "_\\d{6}_0p05_masked\\.tif$"),
      full.names = TRUE
    )
    ...
    # Subset rasters
    masked_vals   <- mask(ras_mean,
                          mask_obj,
                          maskvalues = 0,
                          inverse = TRUE)  # where mask == 1 (drop)
    unmasked_vals <- mask(ras_mean,
                          mask_obj,
                          maskvalues = 1,
                          inverse = TRUE)  # where mask == 0 (keep)
    ...
    write.csv(result, file.path(
      ...
      paste0(
        ...
        "_",
        tolower(MASK_NAME),
        "_masked_vs_unmasked.csv"
      )
    ), row.names = FALSE)

    message(
      glue(
        "{VAR}, {MASK_NAME}: mean (unmasked) = {sprintf('%.3f', mean_unmasked)}, mean (masked) = {sprintf('%.3f', mean_masked)}"
      )
    )
