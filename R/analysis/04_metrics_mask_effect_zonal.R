...
# time-mean a raster that may have multiple layers (annual time series)
...
zonal_wmean_latbands <- function(r, A, band_deg = 1L) {
  compareGeom(A, r, stopOnError = TRUE)
  lat <- init(r, "y")
...
z_unm <- zonal_wmean_latbands(U_amp, A0, band_deg = BAND_DEG) |>
  mutate(domain = lab_unm)

z_nat <- zonal_wmean_latbands(N_amp, A0, band_deg = BAND_DEG) |>
