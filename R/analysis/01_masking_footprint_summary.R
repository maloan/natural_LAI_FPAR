# ==============================================================================
# 01_masking_footprint_summary.R — Land-use footprints within the valid land
# domain
# ==============================================================================
...
# compute component areas from 0.05° conditions
summarise_005 <- function(m_lu, m_luh, label, run_tag) {
  ...
  partition <- tibble::tibble(
    ...
    km2 = km2_vals,
    pct_of_land_after_abi = 100 * km2_vals / s$area_dom_total
  )
  list(compact = compact, partition = partition)
}

component_area_global <- function(lu_005, luh_005) {
  # Ensure masks are binary-safe by converting NA values to 0
...
s_glc <- summarise_005(ms_glc$lu, ms_glc$luh, "GLC", glc_run_tag)
...
write_csv(
  round_numeric(tbl_land, 5),
  file.path(
    outdir,
    "domain_landuse_after_nonvegetated_overview_0p05.csv"
  )
)
write_csv(
  round_numeric(tbl_overlap, 5),
  file.path(outdir, "domain_overlap_partition_overview_0p05.csv")
)
