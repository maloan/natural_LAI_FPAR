...
top_n <- Inf # number of 2-letter groups in plot
...
write.csv(
  kg_full,
  file.path(out_dir, sprintf("kg_class_full_%s_%s_%s_%s.csv", var, metric, mask, suffix)),
  row.names = FALSE
)
...
kg_tab3 <- kg_full |>
  ...
  dplyr::summarise(
    num_unm = sum(mean_unmasked * (area_unm_mkm2 * 1e6), na.rm = TRUE),
    den_unm_km2 = sum(area_unm_mkm2 * 1e6, na.rm = TRUE),
    num_msk = sum(mean_masked * (area_msk_mkm2 * 1e6), na.rm = TRUE),
    den_msk_km2 = sum(area_msk_mkm2 * 1e6, na.rm = TRUE),
    num_out = sum(mean_masked_out * (area_out_mkm2 * 1e6), na.rm = TRUE),
    den_out_km2 = sum(area_out_mkm2 * 1e6, na.rm = TRUE),
    .groups = "drop"
  ) |>
  dplyr::mutate(
    mean_unmasked   = num_unm / den_unm_km2,
...
write.csv(
  kg_tab3,
  file.path(out_dir, sprintf("kg_code3_%s_%s_%s_%s.csv", var, metric, mask, suffix)),
  row.names = FALSE
)
...
yspan3 <- diff(range(plot_df3$value, na.rm = TRUE))
yoff3 <- 0.03 * ifelse(is.finite(yspan3) && yspan3 > 0, yspan3, max(abs(plot_df3$value), na.rm = TRUE))
...
kg_tab2 <- kg_full |>
  ...
  dplyr::summarise(
    num_unm = sum(mean_unmasked * (area_unm_mkm2 * 1e6), na.rm = TRUE), # back to numerator
    den_unm_km2 = sum(area_unm_mkm2 * 1e6, na.rm = TRUE),
    num_msk = sum(mean_masked * (area_msk_mkm2 * 1e6), na.rm = TRUE),
    den_msk_km2 = sum(area_msk_mkm2 * 1e6, na.rm = TRUE),
    num_out = sum(mean_masked_out * (area_out_mkm2 * 1e6), na.rm = TRUE),
...
write.csv(
  kg_tab2,
  file.path(out_dir, sprintf("kg_code2_%s_%s_%s_%s.csv", var, metric, mask, suffix)),
  row.names = FALSE
)
