...
for (f in files) {
  ...
  if (substr(ym, 5, 6) %in% c("01", "07")) {
    ...
    if (REMAKE_QL || !(SKIP_EXISTING && file.exists(ql_png))) {
      quicklook_before_after(r, r_masked, ym, title = ql_title, ql_dir = ql_dir)
    }
    ql_full <- file.path(
      out_dir,
      "quicklooks",
    ...
    if (REMAKE_QL || !(SKIP_EXISTING && file.exists(ql_full))) {
      quicklook_after_full(r_masked, ym, title = ql_title, ql_dir = ql_dir)
    }
  }
  ...
  for (nm in names(aois)) {
    ...
    if (inherits(r_aoi_before, "try-error") ||
        inherits(r_aoi_after, "try-error")) {
      next
    }
    ...
    if (!REMAKE_QL && SKIP_EXISTING && file.exists(ofile_ba)) {
      ...
      png(ofile_ba,
          ...
          res = 120)
      par(mfrow = c(1, 2),
          mai = c(0.6, 0.6, 0.7, 0.6))
      plot(r_aoi_before, main = sprintf("%s Before (%s)\nAOI: %s", VAR, ym, nm))
      plot(
        st_geometry(coast),
        add = TRUE,
        col = "black",
    ...
    ofile_after <- file.path(aoi_dir, sprintf("%s_%s_masked_aoi_%s.png", VAR, ym, nm))
