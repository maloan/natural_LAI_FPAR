#!/usr/bin/env Rscript
## =============================================================================
# 07_luh_masks.R — LUH pasture & rangeland shares (0.25° + 0.05° replicas)


suppressPackageStartupMessages({ library(terra); library(yaml); library(glue) })
...
if (!file.exists(luh_nc)) stop("LUH not found: ", luh_nc)
...
if (!length(idx)) stop("No LUH steps in ", Y0, "-", Y1)

pas_mu <- mean(pas[[idx]], na.rm=TRUE)
rng_mu <- mean(rng[[idx]], na.rm=TRUE)

# shares at 0.25°
m025_pas <- clamp(pas_mu, 0, 1); names(m025_pas) <- "pasture_share"
m025_rng <- clamp(rng_mu, 0, 1); names(m025_rng) <- "rangeland_share"
...
writeRaster(m025_pas, f_pas_025, overwrite=TRUE, gdal=gdal_wopt("FLT4S")$gdal, NAflag=-9999)
writeRaster(m025_rng, f_rng_025, overwrite=TRUE, gdal=gdal_wopt("FLT4S")$gdal, NAflag=-9999)
...
if (WRITE_005) {
  ...
  if (!compareGeom(m005_pas, ref005, stopOnError=FALSE)) m005_pas <- resample(m005_pas, ref005, method="near")
  if (!compareGeom(m005_rng, ref005, stopOnError=FALSE)) m005_rng <- resample(m005_rng, ref005, method="near")
  ...
  writeRaster(m005_pas, f_pas_005, overwrite=TRUE, gdal=gdal_wopt("FLT4S")$gdal, NAflag=-9999)
  writeRaster(m005_rng, f_rng_005, overwrite=TRUE, gdal=gdal_wopt("FLT4S")$gdal, NAflag=-9999)
}
...
plot_ql <- function(r, title, file, zlim=c(0,1), pal=pal_col){
  png(file, 1400, 700, res=120); op <- par(mar=c(3,3,3,6))
  terra::plot(r, main=title, col=pal, zlim=zlim, axes=TRUE, legend=TRUE)
  mtext("0..1 fraction", 3, line=0.7, cex=0.9); par(op); dev.off()
}

plot_ql(m025_pas, sprintf("LUH pasture %s (0.25°)", tag),
        file.path(ql_dir, sprintf("pasture_%s_0p25.png", tag)))
plot_ql(m025_rng, sprintf("LUH rangeland %s (0.25°)", tag),
        file.path(ql_dir, sprintf("rangeland_%s_0p25.png", tag)))

if (WRITE_005) {
  plot_ql(m005_pas, sprintf("LUH pasture %s (0.05° replica)", tag),
          file.path(ql_dir, sprintf("pasture_%s_0p05_rep.png", tag)))
  plot_ql(m005_rng, sprintf("LUH rangeland %s (0.05° replica)", tag),
