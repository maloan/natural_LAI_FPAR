#!/usr/bin/env Rscript
# 04_luh_pasture_overlap_025.R — 0.25° LUH pasture overlap mask + 0.05° replica

suppressPackageStartupMessages({ library(terra); library(yaml); library(glue); library(stringr) })
# --- config & refs -------------------------------------------------------------
# Root dir
...
get_int_env <- function(name, fallback) {
  v <- Sys.getenv(name, NA_character_)
  if (is.na(v) || !nzchar(v)) return(as.integer(fallback))
...
stopifnot(is.finite(G_MIN), is.finite(P_MIN), is.finite(ALPHA), is.finite(Y0), is.finite(Y1))
...
grass_005 <- switch(
  ...
  "GLC_FRAC" = {
    ...
    if (!file.exists(p)) stop("GLC_FRAC path not found: ", p)
    rast(p)
  },
  "GLC_TEMP" = {
...
if (!(v_pas %in% sds_names))
  stop("LUH subdataset '", v_pas, "' not found. Available: ", paste(sds_names, collapse=", "))

pas <- rast(luh_nc, subds=v_pas)
ty  <- suppressWarnings(as.integer(time(pas)))
if (all(is.na(ty))) stop("LUH time axis missing/NA for '", v_pas, "'. Check file.")
...
pasture_025 <- clamp(pasture_025, 0, 1); names(pasture_025) <- "pasture"
...
fname <- sprintf("mask_luh_overlap_%s_Gmin%s_Pmin%s_alpha%s_%d-%d_0p25.tif",
                 GRASS_SOURCE, tok(G_MIN), tok(P_MIN), tok(ALPHA), Y0, Y1)
out025 <- file.path(out_dir, fname)
writeRaster(mask_025, out025, overwrite=TRUE, gdal=gdal_wopt("LOG1S")$gdal, NAflag=255)
...
writeRaster(mask_005, out005, overwrite=TRUE, gdal=gdal_wopt("LOG1S")$gdal, NAflag=255)

# --- 6) quicklooks -------------------------------------------------------------
if (REMAKE_QL || !file.exists(...)){
png(file.path(ql_dir, sub("\\.tif$",".png", basename(out025))), 1400, 700, res=120)
op <- par(mfrow=c(1,3), mar=c(3,3,3,6))

terra::plot(grass_025,   main="Grass fraction (0.25°)",   col=pal_grass,  zlim=c(0,1))
terra::plot(pasture_025, main="Pasture fraction (0.25°)", col=pal_pasture, zlim=c(0,1))
terra::plot(mask_025,    main=sprintf("Mask (α=%s; G≥%s; P≥%s)", tok(ALPHA), tok(G_MIN), tok(P_MIN)),
            col=c("#f0f0f0","#d73027"), breaks=c(-0.5,0.5,1.5), legend=FALSE, axes=TRUE, box=TRUE)
legend("bottomleft", fill=c("#f0f0f0","#d73027"), legend=c("0 keep","1 drop"), bty="n")
par(op); dev.off()
}
cat(glue("
Wrote:
