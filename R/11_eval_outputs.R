## =============================================================================
# 11_eval_outputs.R — Visual summaries of mask performance and sensitivity
#
# Purpose
#   Produce graphical diagnostics from evaluation outputs, including global
#   masked fractions, pairwise overlaps, and sensitivity curves for each
#   masking method (CCI, GLC, LUH). Outputs concise figures for QA and reports.
#
# Inputs
#   CSVs from 10_eval_masks.R:
#     - global_masked_fraction.csv
#     - pairwise_comparison_area_weighted.csv
#     - sensitivity_cci_tau_k.csv
#     - sensitivity_glc_N.csv
#     - sensitivity_luh_alpha_gmin_pmin.csv
#
# Outputs
#   - fig_masked_fraction.png — area fraction removed by mask (bar)
#   - fig_pairwise_jaccard.png — mask agreement (Jaccard)
#   - fig_pair_heat_CCI_vs_GLC.png — contingency heatmap (CCI vs GLC)
#   - fig_sens_cci_tau_k.png — τ/k sensitivity (CCI)
#   - fig_sens_glc_N.png — N sensitivity (GLC)
#   - fig_sens_luh_heatmap.png — α–Pmin heatmap (LUH; fixed Gmin)
#
# Dependencies
...
#
# Processing overview
#   1) Plot global masked fractions as percentage bars.
#   2) Visualize pairwise Jaccard overlap and contingency structure.
#   3) Plot τ–k sensitivity for CCI masks.
#   4) Plot N sensitivity for GLC masks.
#   5) Plot α–Pmin heatmap for LUH overlap masks.
## =============================================================================
...
read_csv(file.path(OUTDIR, "global_masked_fraction.csv")) |>
  ...
  geom_col(width = 0.6) +
  scale_y_continuous(labels = percent_format(accuracy = 0.1), expand = expansion(mult =
                                                                                   c(0, 0.05))) +
  ...
  labs(title = "Area fraction removed by each mask", x = NULL, y = "Area removed (%)") +
  theme_minimal(base_size = 12) + theme(legend.position = "none")
ggsave(
  file.path(OUTDIR, "fig_masked_fraction.png"),
  width = 5.5,
  height = 3.6,
  dpi = 200
)
...
pair |>
  ...
  labs(title = "Agreement on dropped area (Jaccard index)", x = NULL, y =
         "Jaccard (drop=1)") +
  theme_minimal(base_size = 12) + theme(legend.position = "none")
ggsave(
  file.path(OUTDIR, "fig_pairwise_jaccard.png"),
  width = 6,
  height = 3.6,
  dpi = 200
)
...
mk_heat <- function(df, which_pair, outfile) {
  ...
  tibble(
    ...
    labs(
      title = paste0(which_pair, ": area-weighted contingency"),
      x = "Other mask",
      y = "CCI"
    ) +
...
mk_heat(pair,
        "CCI_vs_GLC",
        file.path(OUTDIR, "fig_pair_heat_CCI_vs_GLC.png"))
...
ggsave(
  file.path(OUTDIR, "fig_sens_cci_tau_k.png"),
  width = 6.2,
  height = 3.8,
  dpi = 200
)
...
glc |>
  ...
  theme_minimal(base_size = 12)
ggsave(
  file.path(OUTDIR, "fig_sens_glc_N.png"),
  width = 6.2,
  height = 3.8,
  dpi = 200
)
...
luh |> filter(abs(gmin - 0.05) < 1e-6) |>
  ...
  labs(
    title = "LUH pasture-overlap sensitivity (Gmin=0.05)",
    x = expression(alpha ~  ~ "(pasture / grass)"),
    y = expression(P[min])
  ) +
  theme_minimal(base_size = 12)
ggsave(
  file.path(OUTDIR, "fig_sens_luh_heatmap.png"),
  width = 6.2,
  height = 4.6,
  dpi = 200
)
