...
#
# Purpose
#   Shared tools for raster alignment, CF-compliant time extraction, and
#   area-weighted aggregation used across preprocessing and evaluation steps.
#
# Functions
...
#   rotate_if_360()           — Convert 0–360° → −180–180° longitudes
#   .extract_cf_times()       — Parse CF-style time variable from NetCDF
#   extract_time_slice()      — Extract nearest CF-time slice by YYYYMM
#   agg005_to_025_aw()        — Area-weighted 0.05° → 0.25° aggregation
#   stack_aligned()           — Load and align rasters to a template
...
# ------------------------------------------------------------------------------
# Grid comparison and lon/lat helpers
# ------------------------------------------------------------------------------
...
maybe_flip_lat <- function(arr, lat_vals) {
  if (is.null(lat_vals) || length(dim(arr)) != 2L) return(arr)
...
.extract_cf_times <- function(nc) {
  ...
  if (inherits(units, "try-error") || !is.character(units)) return(NULL)
  ...
  step   <- mm[2]
  origin <- as.POSIXct(
    paste0(mm[3], if (!grepl(":", mm[3])) " 00:00:00" else ""),
    tz = "UTC"
  )
...
extract_time_slice <- function(arr, varnm, nc, ym_fallback) {
  if (length(dim(arr)) <= 2L) return(arr)
  ...
  if (length(tpos) && !is.null(dates)) {
    ym_target <- try(
      as.Date(paste0(ym_fallback, "01"), "%Y%m%d"),
      silent = TRUE
    )
...
# ------------------------------------------------------------------------------
# Area-weighted aggregation and stacking helpers
# ------------------------------------------------------------------------------
...
agg005_to_025_aw <- function(r005, area005, ref025) {
  ...
  if (!all(abs(c(fx, fy) - round(c(fx, fy))) < 1e-9)) {
    stop(sprintf(
      "Grids are not nested: ref025/res(r005) = (%.8f, %.8f)",
      fx, fy
    ))
  }
